...
#Script takes the data from python script 024
#Loading data
current_path = getwd();
setwd("../Python/026_analysis_draws_until_all_colors__colorless_lands__with_monte_carlo/Data_colorless");#
setwd("../Python/026_analysis_draws_until_all_colors__colorless_lands__with_monte_carlo/");#

files <- list.files(
  ".",
  pattern = "colorless_lands__MC_simulation\\.txt$",
  full.names = TRUE
)
...
mean(diff(result.starting.2.df$starting_ava*100));
...
positions <- c(0, 1, 2, 3, 4, 5, 10, 15, 20, 25, 30);
...
write.table(
  comparisons.df,
  "7_6_1_table_results_average_turns.txt",
  sep = "\t",
  row.names = FALSE,
  quote = FALSE,dec = ","
)
...
for (result.df in results_all_colors){
  ...
  Simplot = ggplot( result.df, aes( x = Trial_land_count, y = mean_turns))+
    ...
    scale_x_continuous( name = "Colorless lands added to mana base.",
...
color.3.starting.hand.plot = ggplot(result.starting.3.df, aes(x = Trial_land_count, y=starting_ava))+
  ...
  scale_y_continuous(name = "Percentage of hands with all colors.")+
  background_grid()+
  theme(legend.position = "none");
...
plot_name = paste("Chapter_7_6_1_simulation_average_turns__adding_fetch_lands__plot__", Sys.Date()  ,".png");
ggsave(plot_name, plot = All_average_turn_plots, 
       width =10, height =10, dpi = 300);


plot_name = paste("Chapter_7_6_2__3_color_improvement_starting_colors__fetches__plot__", Sys.Date()  ,".png");
