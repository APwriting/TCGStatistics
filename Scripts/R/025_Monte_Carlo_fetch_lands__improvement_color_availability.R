...
#Loading data
current_path = getwd();
setwd("../Python/024_fetch_land_basic_monte_carlos");#
files <- list.files(
  ".",
  pattern = "fetch_lands__MC_simulation\\.txt$",
  full.names = TRUE
)
...
write.table(
  comparisons.df,
  "7_5_1_table_results_average_turns.txt",
  sep = "\t",
  row.names = FALSE,
  quote = FALSE,dec = ","
)
...
plot_name = paste("Chapter_7_5_1_simulation_average_turns__adding_fetch_lands__plot__", Sys.Date()  ,".png");
ggsave(plot_name, plot = All_average_turn_plots, 
       width =10, height =10, dpi = 300);


plot_name = paste("Chapter_7_5_2__3_color_improvement_starting_colors__fetches__plot__", Sys.Date()  ,".png");
