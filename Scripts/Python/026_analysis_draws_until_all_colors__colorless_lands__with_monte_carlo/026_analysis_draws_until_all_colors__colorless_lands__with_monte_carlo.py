...
def main():
    ...
    file_name = "026_Saved_run_for_plotting__{}_colors.txt".format(colors)
    ...
    for sample_size in range( lower_range, higher_range ):
        ...
        for round in range(repertitions ):
          ...
          #  proxy = input()
            ...
            turn_file_path = Path("026_turns_until_all_colors__{}__colorless_lands__MC_simulation.txt".format(colors))
...
def add_colorless_land_to_land_base(Basic_color_land_base, lands_to_add):
    #Add colorless lands. Identifier for this is defined in the function
    Basics  = sorted( [ str(ele) for ele in list(Basic_color_land_base.keys()) ] )

    #Defining all color land
    COLORLESS_land_ID = "COLORLESS"

    Current_Basic_to_fix_ID = 0
    for i in range(lands_to_add):
        Basic_color_land_base[COLORLESS_land_ID] += 1
